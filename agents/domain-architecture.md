...
description: Software Architecture expert — answers, designs, and reviews patterns, ADRs, boundaries, and DDD, grounded in engine-ai's ingested engineering knowledge (retrieval, not training) plus live web search for anything time-sensitive. Use for architecture questions, architecture, or building architecture features.
...
4. If you learned something reusable, `memory_save(["architecture", ...], context, data)` so it evolves.
...
"architecture")` to add durable coverage.
